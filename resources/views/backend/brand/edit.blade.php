                        ...
                        <form action="{{ route('admin.brands.update', $brand->id) }}" enctype="multipart/form-data" method="post">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-4 col-md-6">
                                    ...
                                    <input type="text" style="background-color: #f6f6f6;" readonly class="form-control" name="slug" id="slug" value="{{ $brand->slug }}" required >
                                ...
                                <div class="mb-4 col-md-6">
                                    <label class="form-label" for="image">Image (width:106px height:50px) <i class="text-danger">*</i></label>
                                    <input type="file" class="form-control" accept="image/*" name="image" id="image" >
                                    <span class="text-danger">{{ $errors->has('image') ? $errors->first('image') : '' }}</span>
                                    @if($brand->image != '' && Storage::disk('public')->exists($brand->image))
                                        <a class="mt-2 d-block" target="_blank" href="{{ asset('storage/'.$brand->image) }}">Show Image</a>
                                    @endif
                                </div>
